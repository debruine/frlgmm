...
readTems <- function(tem.loc, semi.landmarks = 0, curvefile = "", analyse = TRUE) {
  oldwd <- getwd()
  ...
  mydata <- list(land, curvepts, teminfo)
  names(mydata) = c("land", "curvepts", "info")

  if (analyse) {
    mydata$aligned <- procrustesAlign(mydata)
    mydata$pca <- getPCA(mydata)
    mydata$data <- getData(mydata, selectPCs(mydata))
  }
