...
graphMinMax <- function(data, pc = 1, mag = 1, method = "TPS") {
  ...
  PCA <- plotTangentSpace(coords, axis1 = pc, axis2 = pc, verbose = T, warpgrids = FALSE)
  ...
  plotRefToTarget(mean.shape, PCA$pc.shapes[[1]], mag = mag, method = method)
  plotRefToTarget(mean.shape, PCA$pc.shapes[[2]], mag = mag, method = method)
