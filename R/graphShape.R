...
#' face.sd3.max <- graphShape(faces, c(0,0,2), show.image = F,
#'                            savename="sd3max.tem", 
#'                            sampletem=faces$info$name[1],
#'                            align.to = mshape(faces$land))

graphShape <- function(data, set.pc = c(0), by = "SD", show.image = F, 
                       save.name = "", sample.tem = "", align.to = NULL) {
  ...
  pc.vis <- arrayspecs(as.matrix(pc.vec %*% (t(data$pca$rotation))), p, k)[, , 1] + mean.shape
  ...
  # save to a file
  if (save.name != "") {
    saveTem(coords = pc.vis, 
            save.name = save.name, 
            sample.tem = sample.tem, 
            align.to = align.to)
  }
